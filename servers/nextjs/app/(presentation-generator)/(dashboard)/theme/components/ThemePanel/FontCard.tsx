...
export const FontCard: React.FC<FontCardProps> = ({ font, isSelected, onSelect }) => (
  <div
    className={`relative p-3 rounded-xl cursor-pointer transition-all duration-200 group
      ${isSelected
        ? 'bg-gradient-to-br from-[#F8F7FF] to-[#F0EFFF] border border-[#00C278] shadow-sm'
  ...
  >
    ...
    <div className="flex items-center justify-between gap-2">
      <div className="flex-1 min-w-0">
        <p
          className={`text-sm font-medium truncate ${isSelected ? 'text-[#00C278]' : 'text-[#151515]'}`}
      ...
      <div
        className={`text-xl font-semibold ${isSelected ? 'text-[#00C278]' : 'text-[#333] group-hover:text-[#00C278]'} transition-colors`}
        style={{ fontFamily: `"${font.name}"` }}
      >
        Aa
      </div>
