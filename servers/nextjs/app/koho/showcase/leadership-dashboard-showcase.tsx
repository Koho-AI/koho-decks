...
const locationPerf = [
  { name: 'Kings Cross', mrr: '£98k', occupancy: 91, nps: 68, status: 'healthy' as const },
  { name: 'Shoreditch', mrr: '£82k', occupancy: 88, nps: 65, status: 'healthy' as const },
  { name: 'Manchester', mrr: '£64k', occupancy: 79, nps: 58, status: 'warning' as const },
  { name: 'Bristol', mrr: '£43k', occupancy: 84, nps: 62, status: 'healthy' as const },
  { name: 'Edinburgh', mrr: '£38k', occupancy: 82, nps: 66, status: 'healthy' as const },
  { name: 'Birmingham', mrr: '£52k', occupancy: 76, nps: 54, status: 'warning' as const },
  { name: 'Leeds', mrr: '£35k', occupancy: 80, nps: 60, status: 'healthy' as const },
  { name: 'Cambridge', mrr: '£28k', occupancy: 86, nps: 70, status: 'healthy' as const },
];

export function LeadershipDashboardShowcase() {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className={cn('text-[9px] font-medium tabular-nums', good ? 'text-[hsl(var(--status-healthy))]' : 'text-[hsl(var(--status-danger))]')}>
                      {kpi.delta > 0 ? '+' : ''}{kpi.delta}{kpi.suffix === 'x' ? 'x' : '%'}
                    </span>
                  </div>
                </motion.div>
              );
            ...
            >
              <div className="flex items-center justify-between border-b border-border/30 px-3 py-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-semibold">Quarterly MRR vs Target</span>
                </div>
                <span className="text-[9px] text-muted-foreground">FY24</span>
              </div>
              <div className="min-h-0 flex-1 p-2">
                <ChartContainer config={chartConfig} className="h-full w-full">
                  <BarChart data={quarterlyMRR} margin={{ top: 4, right: 4, bottom: 0, left: -10 }}>
                    <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="hsl(var(--border)/0.3)" />
                    <XAxis dataKey="quarter" tickLine={false} axisLine={false} tick={{ fontSize: 9, fill: 'hsl(var(--muted-foreground))' }} />
                    <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 9, fill: 'hsl(var(--muted-foreground))' }} tickFormatter={(v: number) => `${v / 1000}k`} width={28} />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Bar dataKey="actual" fill="var(--color-actual)" radius={[3, 3, 0, 0]} maxBarSize={24} />
                    <Bar dataKey="target" fill="var(--color-target)" fillOpacity={0.45} radius={[3, 3, 0, 0]} maxBarSize={24} />
                  </BarChart>
                </ChartContainer>
              </div>
            </motion.div>
                ...
                {locationPerf.map((loc, i) => (
                  ...
                  >
                    ...
                    <div className="relative h-2.5 flex-1 overflow-hidden rounded-full bg-muted/50">
                      <motion.div
                        className={cn('absolute inset-y-0 left-0 rounded-full', loc.status === 'healthy' ? 'bg-[hsl(var(--status-healthy)/0.5)]' : 'bg-[hsl(var(--status-warning)/0.5)]')}
                        initial={{ width: 0 }}
            ...
            <table className="w-full text-[10px]">
              <thead>
                <tr className="border-b border-border/20 text-muted-foreground">
                  ...
                  <th className="hidden px-3 py-1.5 text-right font-medium sm:table-cell">NPS</th>
              ...
              <tbody>
                {locationPerf.map((row, i) => (
                  <motion.tr key={row.name} className="border-b border-border/10 last:border-b-0" initial={animate ? { opacity: 0 } : false} animate={animate ? { opacity: 1 } : undefined} transition={{ delay: 0.8 + i * 0.05, duration: 0.2 }}>
                    ...
                    <td className="px-3 py-1.5 text-right tabular-nums">{row.occupancy}%</td>
                    <td className="hidden px-3 py-1.5 text-right tabular-nums sm:table-cell">{row.nps}</td>
                    <td className="px-3 py-1.5">
                      <span className={cn('inline-flex items-center rounded-full px-1.5 py-0.5 text-[8px] font-medium', row.status === 'healthy' ? 'bg-[hsl(var(--status-healthy)/0.1)] text-[hsl(var(--status-healthy))]' : 'bg-[hsl(var(--status-warning)/0.1)] text-[hsl(var(--status-warning))]')}>
                        {row.status === 'healthy' ? 'On Track' : 'Watch'}
                      </span>
                    </td>
                  </motion.tr>
                ))}
